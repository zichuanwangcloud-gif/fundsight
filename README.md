...
- **零第三方依赖（核心）**：识别层只用 Python 标准库，外部调用收敛在
  `backend/datasource/vision_ocr.py`（唯一对外接口）。识别引擎二选一（均为可选）：

  **通道 A · 本地 OCR（截图不出本机，合规更稳，推荐自用）**
  ```bash
  pip install rapidocr_onnxruntime      # 纯 pip、无需系统二进制、中文较准
  export FUNDSIGHT_VISION_PROVIDER=local
  ```
  纯本地识别，截图不出网，天然满足「仅自用私享」红线；代价是版面解析靠启发式，
  准确度略低于大模型（故一律经确认页人工校正）。

  **通道 B · 云端视觉大模型（识别更准，截图会发往所配服务）**
  ```bash
...
- **合规与隐私**：通道 A 全程本地、截图不出网；通道 B 会把截图（含金额/收益等财务数据）
  发送到你**已配置的**服务，仅自用私享场景由你自行知情授权。两通道截图均仅内存处理，
  **不落库、不留存**（延续「仅自用私享」红线）。
