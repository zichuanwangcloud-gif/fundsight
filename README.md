...
- **零第三方依赖**：识别只用 Python 标准库 `urllib` 直接调多模态视觉大模型 HTTP API，
  不引入任何 `pip` 依赖（外部调用收敛在 `backend/datasource/vision_ocr.py`，唯一对外接口）。
- **可配置识别引擎**：默认走 Claude，也可指向任意 OpenAI 兼容 / 自建视觉服务：
...
- **优雅降级**：未配置密钥时功能自动降级 —— 上传入口给出配置提示，手动搜索录入不受影响。
- **识别必经确认**：识别不可能 100% 准，结果先进确认页由你核对/改正才写库，绝不静默入库。
- **合规与隐私**：识别会把截图（含金额/收益等财务数据）发送到你**已配置的**识别服务，
  仅自用私享场景由你自行知情授权；截图仅在内存处理，**不落库、不留存**（延续「仅自用私享」红线）。
